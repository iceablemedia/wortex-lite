...
function wortex_settings_admin_scripts() {
	wp_register_style( 'wortex_admin_css', get_template_directory_uri() .'/functions/icefit-options/style.css');
	wp_enqueue_style( 'wortex_admin_css' );
	wp_enqueue_script( 'wortex_admin_js', get_template_directory_uri() . '/functions/icefit-options/functions.js', array( 'jquery' ), false, true );
}
...
function wortex_settings_machine($options) {
	...
	foreach ($options as $arg) {
		...
		elseif ( $arg['type'] == "image" )
		{
			...
			$output .= '<input class="wortex_upload_button" name="'. $arg['id'] .'_upload" id="'. $arg['id'] .'_upload" type="button" value="Upload Image">'."\n";
			$output .= '<input class="wortex_remove_button" name="'. $arg['id'] .'_remove" id="'. $arg['id'] .'_remove" type="button" value="Remove"><br />'."\n";
			$output .= '<img class="wortex_image_preview" id="'. $arg['id'] .'_preview" src="'.$val.'"><br class="clear">'."\n";
		}
		elseif ( $arg['type'] == "gopro" )
		{
			$output .= '<h3>'. $arg['name'] .'</h3>'."\n";
			$output .= '<p>Unleash the full potential of Wortex by upgrading to Wortex Pro!</p>';
	...
	<li>Lifetime <strong>free updates</strong> and continued support for the <strong>latest WordPress versions</strong></li>
			...
			$output .= '<a href="http://www.iceablethemes.com/shop/wortex-pro/?utm_source=lite_theme&utm_medium=go_pro&utm_campaign=wortex_lite" class="button-primary" target="_blank">Learn More and Upgrade Now!</a>';
		}
		elseif ( $arg['type'] == "support_feedback" )
		{
			$output .= '<h3>Get Support</h3>'."\n";
			$output .= '<p>Have a question? Need help?</p>';
			$output .= '<p><a href="http://www.iceablethemes.com/forums/forum/free-support-forum/wortex-lite/?utm_source=lite_theme&utm_medium=support_forums&utm_campaign=wortex_lite" target="_blank" class="button-primary">Visit the free Wortex Lite support forums</a></p>';		

			$output .= '<h3>Give Feedback</h3>'."\n";
			$output .= '<p>Like this theme? We\'d love to hear your feedback!</p>';
			$output .= '<p><a href="http://wordpress.org/support/view/theme-reviews/wortex-lite" target="_blank" class="button-primary">Rate and review Wortex Lite at WordPress.org</a></p>';		
	...
	foreach($options as $option_array){
		...
		if ( isset($option_array['id']) && $option_array['type'] != 'start_menu' && $option_array['type'] != 'end_menu' ) {
			...
			if ($option_array['type'] == "text") {
				...
				if (isset($_POST[$option_array['id']]))

 ...
 *
 */
var wortex_admin_panel;
(function(jQuery){wortex_admin_panel={
	init:function(){
		//admin panel
		var icf_title;
		...
		jQuery('#icefitform').submit(function(){
			...
			var data = {
				action: 'wortex_settings_ajax_post_action',
				data: serializedReturn,
				wortex_settings_nonce: jQuery('#wortex_settings_nonce').val()
			};
			jQuery.post(ajaxurl, data);
			jQuery('#ajax-result').html(wortex_js_strings.settings_saved).fadeIn("normal").delay('1000').fadeOut("normal");
			return false; 
		});
		
		// Reset
		jQuery('#icefit-reset-button').click(function() {
			var answer = confirm(wortex_js_strings.reset_confirm);
			if (answer) {
				var data = { action: 'wortex_settings_reset_ajax_post_action' };
				jQuery.post(ajaxurl, data);
